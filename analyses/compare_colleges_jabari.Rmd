---
title: "College Comparison - Jabari"
output: 
  html_document:
    theme: cosmo
---
...
jabari_options <-
  ...
  select(
    ...
    net_price.public.by_income = `net_price.public.by_income_level.0-48000`,
    net_price.private.by_income = `net_price.private.by_income_level.0-48000`,
  ...
  rowwise() %>%
  mutate(
    major_psych = sum(c_across(matches("psych|neuro")), na.rm = T),
    major_architect = sum(c_across(matches("architect")), na.rm = T),
    major_engineer = sum(c_across(matches("engineer")), na.rm = T),
    major_stats = sum(c_across(matches("statistic")), na.rm = T),
    major_economist = sum(c_across(matches("economic")), na.rm = T)
  ) %>%
  ungroup() %>%
...
pal <- colorNumeric(palette = "RdYlGn", domain = jabari_options$ntile_cost, reverse = T)
